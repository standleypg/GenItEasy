...
-  **Flexible Path Resolution**: Output paths resolve relative to working directory; assembly paths resolve relative to `baseDirectory`
...
var config = ConfigLoader.LoadConfig("path/to/config.json");
...
- Verify the DLL is in the `baseDirectory` or use `--base-directory` to point to the build output folder (e.g., `bin/Debug/net9.0`)
